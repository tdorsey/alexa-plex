...
function query(api, options) {
    ...
    return api.query(options).then(function(response) {
        ...
        //console.log("query: " + options + " response: " + JSON.stringify(response));
        return response;
    })
}
...
var getPlayers = function(app) {
    return query(app.plex.pms, '/clients').then(function(resources) {
        return resources.MediaContainer.Server.filter(function(resource){
...
var startShow = function(app, options, response) {
    console.log("starting show with options: " + JSON.stringify(options));
    ...
    return getListOfTVShows(app, app.user.TVLibrary).then(function(listOfTVShows) {
        ...
        console.log("found best match show: " + show.title + " for spokenShowName: " + spokenShowName + " with confidence: " + matchConfidence);
        ...
        return getAllEpisodesOfShow(app, show).then(function (allEpisodes) {
            ...
            if(episodeNumber || seasonNumber) {
                console.log("looking for episode: " + episodeNumber + " or season: " + seasonNumber);
                ...
                var seasonEpisodes = allEpisodes.MediaContainer.Metadata.filter(function(ep) {return ep.parentIndex==seasonNumber;});
                ...
                console.log("forceRandom not set, finding first unwatched");
                episode = getFirstUnwatched(allEpisodes.MediaContainer.Metadata);
            ...
            if(!episode) {
                console.log("no episode found yet, looking for partially-watched episode");
                // First check to see if there's a partially-watched episode in this show
                episode = findEpisodeWithOffset(allEpisodes.MediaContainer.Metadata, onlyTopRated);
                ...
                if(episode) {
                    ...
                    var randomEpisode = getRandomEpisode(allEpisodes.MediaContainer.Metadata, onlyTopRated);
                ...
                episode = getRandomEpisode(allEpisodes.MediaContainer.Metadata, onlyTopRated);
...
var getListOfTVShows = function(app, library) {
    console.log("querying library key: " + library.key);
...
var playMedia = function(app, parameters) {
    ...
    console.log("media uri: " + playMediaURI);

    var playerID = app.user.player.machineIdentifier;
    console.log("playerID: " + playerID);

    var options = { uri: playMediaURI, extraHeaders: {'X-Plex-Target-Client-Identifier': playerID} };

    return perform(app.plex.pms, options);
};

var getPlayerAddress = function(app){
    //return app.user.player.address;
    return app.user.hostname;
};

var getPlayerPort = function(app){
    return app.user.port;
};
...
var getShowNamesFromList = function(apiResult) {
    ...
    for(i = 0; i < apiResult.MediaContainer.Directory.length && i < 6; i++) {
        if(apiResult.MediaContainer.Directory[i].type == 'episode') {
            shows.push(apiResult.MediaContainer.Directory[i].grandparentTitle);
        } else if (apiResult.MediaContainer.Directory[i].type == 'movie') {
            shows.push(apiResult.MediaContainer.Directory[i].title);
        }
    }
