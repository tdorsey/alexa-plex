...
var initSkillState = function(app, state) {
    if (!states[state]) {
    ...
    return runSetup(app, state);
};

function runSetup(app, state) {
    if(typeof states[state]['setup'] === 'function') {
        return states[state].setup(app);
    } else {
        return Q.resolve();
    }
}
...
function appPre(request, response, type) {
    if(process.env.ALEXA_APP_ID) {
        if (request.sessionDetails.application.applicationId != "amzn1.ask.skill." + process.env.ALEXA_APP_ID) {
            //fail silently to prevent breaking tests for now
...
function appError(error, request, response) {
    ...
    response
        .clear()
        .say("I'm sorry, but there was an error in the Plex Skill. The error has been logged and sent to the developer to take a look. Feel free to try your request again!")
        .send();
}

module.exports.initSkillState = initSkillState;